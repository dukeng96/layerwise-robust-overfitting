...
def main():
    ...
    import csv
    RESULT_PATH = f"{args.data_dir}/result/version={args.version}_run={args.run}_dataset=svhn_lr={args.lr_schedule}_norm={args.norm}_target_layers={args.target_layers}_model={args.model}.csv"
    ...
    # model = models_dict[args.architecture]().to(device)
    # model.apply(initialize_weights)
    if args.model == 'PreActResNet18':
        model = PreActResNet18()
        proxy = PreActResNet18()
        proxy_proxy = PreActResNet18()
        ...
        proxy_proxy = WideResNet(34, 10, widen_factor=args.width_factor, dropRate=0.0)
    else:
        raise ValueError("Unknown model")

    model = nn.DataParallel(model, device_ids=args.device).to(device)
    proxy = nn.DataParallel(proxy, device_ids=args.device).to(device)
    proxy_proxy = nn.DataParallel(proxy_proxy, device_ids=args.device).to(device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if epoch >= args.awp_warmup:
                ...
                awp = rwp_adversary.calc_awp(inputs_adv=X_adv,
                                             targets=y)
                rwp_adversary.perturb(awp)
        ...
        if not args.eval: 
            ...
            if epoch == 199 and args.target_layers == [3, 4]:
                ...
                }, os.path.join(MODEL_PATH, f'awp_{args.model}_svhn_{args.norm}_{args.target_layers}_last.pth'))
            ...
            if test_robust_acc / test_n > best_test_robust_acc and args.target_layers == [3, 4]:
                torch.save({
                    ...
                    'test_loss': test_loss / test_n,
                    'test_acc': test_acc / test_n,
                }, os.path.join(MODEL_PATH, f'awp_{args.model}_svhn_{args.norm}_{args.target_layers}_best.pth'))
